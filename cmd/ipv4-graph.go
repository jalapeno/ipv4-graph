...
func init() {
	...
	flag.StringVar(&lslink, "edge-name", "ls_link", "Edge Collection name, default \"ls_link\"")
	flag.StringVar(&lsprefix, "prefix-name", "ls_prefix", "Prefix Collection name, default \"ls_prefix\"")
	flag.StringVar(&lsnodeExt, "lsnodeExtended-name", "ls_node_extended", "ls_node_extended Collection name, default: \"ls_node_extended\"")
	flag.StringVar(&ebgpPeer, "bgp-name", "ebgp_peer", "ebgp_peer Collection name, default \"ebgp_peer\"")
	flag.StringVar(&inetPrefixV4, "inetv4-prefix-name", "inet_prefix_v4", "inet prefix v4 Collection name, default \"inet_prefix_v4\"")
	flag.StringVar(&lsv4Graph, "lsv4-graph", "lsv4_graph", "Edge Collection name, default \"lsv4_graph\"")
	flag.StringVar(&ipv4Graph, "ipv4-graph", "ipv4_graph", "Edge Collection name, default \"ipv4_graph\"")
}
...
func main() {
	...
	dbSrv, err := arangodb.NewDBSrvClient(dbSrvAddr, dbUser, dbPass, dbName, lslink,
		lsprefix, lsnodeExt, ebgpPeer,
		inetPrefixV4, lsv4Graph, ipv4Graph, notifier)
	if err != nil {
